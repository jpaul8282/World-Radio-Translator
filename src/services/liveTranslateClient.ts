...
export class LiveTranslateClient {
  ...
  private currentTurnTotalSamples: number = 0;
      ...
      this.ws.onmessage = (event) => {
        try {
          ...
          const commitCurrentTurn = () => {
            ...
            let turnAudioUrl: string | undefined = undefined;
            let turnAudioBlob: Blob | undefined = undefined;

            if (this.currentTurnTotalSamples > 0 && this.currentTurnAudioChunks.length > 0) {
              const merged = new Float32Array(this.currentTurnTotalSamples);
              let offset = 0;
              for (const chunk of this.currentTurnAudioChunks) {
                merged.set(chunk, offset);
                offset += chunk.length;
              }
              const wavBuffer = encodeWAV(merged, 16000);
              turnAudioBlob = new Blob([wavBuffer], { type: "audio/wav" });
              turnAudioUrl = URL.createObjectURL(turnAudioBlob);
            }

            // Reset turn audio accumulators
            this.currentTurnAudioChunks = [];
            this.currentTurnTotalSamples = 0;

            if (orig || trans) {
              const turns = [...this.state.turns];
              const turnId = `clip-${Date.now()}-${Math.floor(Math.random() * 1000)}`;
              turns.push({
                id: turnId,
                ...
                audioFileName: `radio_${turnId}.wav`
              });
              this.updateState({
  ...
  private async startDOMAudioCapture() {
    try {
      ...
      this.micProcessor.onaudioprocess = (e) => {
        ...
        // Accumulate audio chunks for current transcript turn snippet
        this.currentTurnAudioChunks.push(inputData);
        this.currentTurnTotalSamples += inputData.length;
      };
